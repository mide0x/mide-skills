...
import { updateDictionary, unsummarized, invocable } from './dictionary.mjs';
...
export const mentions = (text, name) => new RegExp(`(^|[^a-z0-9_-])${name}(?![a-z0-9_-])`).test(text);
export function dependencies(directory, available, helpers = []) {
  const found = new Map();
  for (const file of files(directory)) {
    const text = fs.readFileSync(file, 'utf8');
    for (const [, name] of text.matchAll(/\.\.\/([a-z0-9][a-z0-9_-]*)\//g)) if (available.includes(name)) found.set(name, 'referenced by relative path');
    for (const name of helpers) if (!found.has(name) && mentions(text, name)) found.set(name, 'a helper skill it reads');
  }
  return found;
}
export function closeDependencies(selected, root, available, helpers = []) {
  const closed = new Set(selected);
  for (const name of closed) {
    for (const [dependency, reason] of dependencies(path.join(root, name), available, helpers)) {
      if (closed.has(dependency)) continue;
      console.log(`${name} needs ${dependency} (${reason}), adding it`);
      closed.add(dependency);
    }
  }
  return [...closed];
}
export function requiredAgents(sheet, skillsRoot, selected, available) {
  const wanted = new Map();
  for (const [, model, effort] of sheet.matchAll(/\bclaude:([a-z0-9.-]+)@(low|medium|high|xhigh|max)\b/g)) {
    const agent = `pstack-${model}-${effort}`;
    if (available.includes(agent)) wanted.set(agent, 'named in config/pstack-models.md');
  }
  for (const skill of selected) {
    const text = files(path.join(skillsRoot, skill)).map(file => fs.readFileSync(file, 'utf8')).join('\n');
    for (const agent of available) {
      if (!wanted.has(agent) && !/^pstack-[a-z0-9.-]+-(low|medium|high|xhigh|max)$/.test(agent) && mentions(text, agent)) wanted.set(agent, `referenced by ${skill}`);
    }
  }
  return wanted;
}
...
export default async function pick(ctx, { source: input, subdir }, prompts) {
  ...
  for (const group of ['skills', 'agents']) {
    ...
    const options = group === 'agents' ? [] : available[group].map(item => {
  ...
  const helpers = available.skills.filter(item => !invocable(fs.readFileSync(path.join(root, 'skills', item, 'SKILL.md'), 'utf8')));
  selected.skills = closeDependencies(selected.skills, path.join(root, 'skills'), available.skills, helpers);
  const agents = requiredAgents(fs.readFileSync(path.join(repo, 'config/pstack-models.md'), 'utf8'), path.join(root, 'skills'), selected.skills, available.agents);
  for (const [agent, reason] of agents) console.log(`Adding agent ${agent} (${reason})`);
  selected.agents = [...agents.keys()];
